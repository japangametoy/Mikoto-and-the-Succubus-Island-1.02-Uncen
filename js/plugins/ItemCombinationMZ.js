...
(() => {
  ...
  const drawItemNameAndDescription = (_window, itemId, y, width) => {
    ...
    _window.drawText(item.name, 0, y + 10, width, 'center');
    _window.makeFontSmaller();
    // Draw description
    const itemDesc1 = item.description.split('\n')[0] || '';
    const itemDesc2 = item.description.split('\n')[1] || '';
    _window.drawText(itemDesc1, 0, y + 116, width, 'center');
    _window.drawText(itemDesc2, 0, y + 142, width, 'center');
  };

  // const drawBiggerIcon = (_window, itemId, y) => {
  //   const item = $dataItems[itemId];
  //   _window.bitmapIcon = new Sprite(ImageManager.loadSystem('Icon'));
  ...
  const drawBiggerIcon = (_window, itemId, y) => {
    const item = $dataItems[itemId];
    _window.bitmapIcon = new Sprite(ImageManager.loadSystem('Icon'));
    // Change the scale to one-fourth as the image is 4 times larger
    const pw = ImageManager.iconWidth;
    const ph = ImageManager.iconHeight;
    _window.bitmapIcon.scale.x = 0.5;
    _window.bitmapIcon.scale.y = 0.5;
    _window.bitmapIcon.x = _window.width / 2 - pw;
    _window.bitmapIcon.y = y + ph * 2 + 2; // 調整: アイコンを少し下に移動
    const sx = item.iconIndex % 16 * pw * 4; // 4倍のサイズに対応
    const sy = Math.floor(item.iconIndex / 16) * ph * 4; // 4倍のサイズに対応
    _window.addChild(_window.bitmapIcon);
    _window.bitmapIcon.setFrame(sx, sy, pw * 4, ph * 4); // 4倍のサイズに対応
  };
  ...
  const drawGuideText = (_window, y) => {
    _window.changeTextColor(_window.systemColor());
    const text = requiredItemText;
    _window.contents.fontSize = 42; // フォントサイズを4倍に設定
    _window.drawText(text, 0, y + 40, _window.width, "center"); // Y位置を下にずらす
    _window.resetFontSettings(); // フォント設定をリセット
    _window.resetTextColor();
  };
  ...
  Scene_CraftingMenu.prototype.commandWindowRect = function() {
    const wx = 500; // X位置
    const wy = this.mainAreaTop() + 50; // Y位置
    const ww = 300; // 幅
    const wh = Graphics.boxHeight - wy - 280; // 高さ
    return new Rectangle(wx, wy, ww, wh);
  };
  ...
  Scene_CraftingMenu.prototype.detailsWindowRect = function() {
    const wx = 800; // X位置
    const wy = this.mainAreaTop() + 50; // Y位置
    const ww = Graphics.boxWidth - wx - 500; // 幅
    const wh = Graphics.boxHeight - wy - 280; // 高さ
    return new Rectangle(wx, wy, ww, wh);
  };
  ...
  Scene_CraftingMenu.prototype.combineWindowRect = function() {
    const ww = 240; // 幅
    const wx = (Graphics.boxWidth - ww) / 2; // X位置
    const wh = this._commandItemWindow.fittingHeight(1); // 高さ
    const wy = Graphics.boxHeight - wh - 150; // Y位置
    return new Rectangle(wx, wy, ww, wh);
  };
  ...
  Scene_CraftingMenu.prototype.onDetailsOk = function() {
    ...
    if ($gameParty.canCombine(recipeKeyName)) {
        ...
        this._commandCombineWindow.activate(); // 合成後にボタンを再度アクティブにする
        this._commandCombineWindow.refresh(); // ウィンドウの内容を更新する
    } else {
        this._commandCombineWindow.activate(); // 必要なアイテムが足りない場合もアクティブにする
    }
};
  ...
  Scene_CraftingMenu.prototype.createMessageWindow = function() {
    this._messageWindow = new Window_Base(new Rectangle(0, 0, 480, 64));
    this._messageWindow.x = (Graphics.boxWidth - this._messageWindow.width) / 2;
    this._messageWindow.y = (Graphics.boxHeight - this._messageWindow.height) / 2;
    this._messageWindow.openness = 0;
    this.addWindow(this._messageWindow);
};

Scene_CraftingMenu.prototype.showMessage = function(text) {
  ...
  setTimeout(() => {
      ...
      this._commandCombineWindow.activate(); // ボタンを再度アクティブにする
  }, 1000);
};
...
Scene_CraftingMenu.prototype.onDetailsOk = function() {
  ...
  if ($gameParty.canCombine(recipeKeyName)) {
      ...
      if (Math.random() < recipeDetails[1]) {
          $gameParty.gainItem($dataItems[itemId], 1);
          this.showMessage("交換しました");
      } else {
          $gameParty.gainItem($dataItems[recipeDetails[2]], 1);
          SoundManager.playBuzzer();
          this.showMessage("失敗しました");
      }
  } else {
      this._commandCombineWindow.activate(); // 必要なアイテムが足りない場合もアクティブにする
  }
