 ...
 *
 * @help エロステータスプラグイン ver 1.0.1
 *
 * イベントのスクリプトでエロステータス画面を作る事が可能に
 *
 *　～開く～
 *　イベントのスクリプトで
 *　SceneManager.push(Scene_aroStatus)
 *　とやるだけ
 *
 *　～設定～
...
function Scene_aroStatus() {
    this.initialize(...arguments);
}

Scene_aroStatus.prototype = Object.create(Scene_Base.prototype);
Scene_aroStatus.prototype.constructor = Scene_aroStatus;

Scene_aroStatus.prototype.initialize = function() {
    Scene_Base.prototype.initialize.call(this);
};
Scene_aroStatus.prototype.create = function() {
    Scene_Base.prototype.create.call(this);

	//data[{},{},,,]ってなってたら空の部分を削る
	var i = CBR.eroStatus.data.length - 1;
	while(CBR.eroStatus.data.length && i && !CBR.eroStatus.data[i]){
	...
	var i = 0;
	while(CBR.eroStatus.data.length && !CBR.eroStatus.data[i]){
		i++
	}
	CBR.eroStatus.pageNow = i;

	var param = PluginManager.parameters("CBR_aroStatus");
...
Scene_aroStatus.prototype.start = function() {
    Scene_MenuBase.prototype.start.call(this);
    this._aroStatusWindow.refresh();
};
Scene_aroStatus.prototype.createEroStatusWindow = function() {
    const rect = this.eroStatusWindowRect();
    this._aroStatusWindow = new Window_aroStatus(rect);
	this._aroStatusWindow.setHandler("cancel", this.popScene.bind(this));
    this.addWindow(this._aroStatusWindow);
};
Scene_aroStatus.prototype.eroStatusWindowRect = function() {
...
Scene_aroStatus.prototype.update = function() {
    Scene_Base.prototype.update.call(this);
};
Scene_aroStatus.prototype.createBackground = function() {
...
Scene_aroStatus.prototype.setBackgroundOpacity = function(opacity) {
    this._backgroundSprite.opacity = opacity;
};
Scene_aroStatus.prototype.popScene = function() {
    FontManager._states["rmmz-mainfont"] = null;
...
//ウィンドウ
function Window_aroStatus() {
    this.initialize(...arguments);
}

Window_aroStatus.prototype = Object.create(Window_Selectable.prototype);
Window_aroStatus.prototype.constructor = Window_aroStatus;

Window_aroStatus.prototype.initialize = function(rect) {
    Window_Selectable.prototype.initialize.call(this, rect);
	this._CBR_drawn = false;
    this.refresh();
};
Window_aroStatus.prototype.itemPadding = function() {
    return 0;
};
Window_aroStatus.prototype.updatePadding = function() {
    this.padding = 0;
};

Window_aroStatus.prototype.colSpacing = function() {
    return 0;
};
Window_aroStatus.prototype.rowSpacing = function() {
    return 0;
};

Window_aroStatus.prototype.update = function() {
	...
	if(CBR.eroStatus.data.length !== 1 && ImageManager.isReady() && FontManager.isReady()){//読み込み終わってたら、結局画像読み終えてない限りリフレッシュする必要なし
		...
		if(n!==undefined && CBR.eroStatus.pageNow != n && CBR.eroStatus.data[n]){//ページの変更だったら
...
Window_aroStatus.prototype.refresh = function() {
...
Window_aroStatus.prototype.drawText = function(text, x, y, maxWidth, h, align) {
    this.contents.drawText(text, x, y, maxWidth, h, align);
};

Window_aroStatus.prototype.open = function() {
    this.refresh();
    Window_Selectable.prototype.open.call(this);
};
//アクティブじゃなくてもキャンセルできるように
Window_aroStatus.prototype.processHandling = function() {
	if (this.isCancelEnabled() && (this.isCancelTriggered() || TouchInput.isCancelled())) {
		return this.processCancel();
	}
};
Window_aroStatus.prototype.loadWindowskin = function() {
