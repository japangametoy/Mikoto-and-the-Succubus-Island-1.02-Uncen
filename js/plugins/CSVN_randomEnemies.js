    ...
    Game_Troop.prototype.lotteryEnemyIds = function() {
        ...
        for (let i = 0; i < troopObjs.length; i++) {
            ...
            if (dEnemy.meta.RandomEnemy) {
                ...
                // 最初のIDを強制的に含める
                enemyId = parseInt(randomEnemyIds[0], 10);
                dEnemies.push($dataEnemies[enemyId]);
                // 残りのIDからランダムに選択
                for (let j = 1; j < randomEnemyIds.length && dEnemies.length < 3; j++) {
                    enemyId = randomEnemyIds[Math.randomInt(randomEnemyIds.length)];
                    if (!dEnemies.includes($dataEnemies[enemyId])) {
                        dEnemies.push($dataEnemies[enemyId]);
                    }
                }
            } else {
                enemyId = troopObjs[i].enemyId;
                if (enemyId != 0 && !dEnemies.includes($dataEnemies[enemyId])) {
                    dEnemies.push($dataEnemies[enemyId]);
                }
            }
        }
        ...
        // 最大出現数を2に制限
        if (dEnemies.length > 2) {
            dEnemies = dEnemies.slice(0, 2);
        }
    ...
    Game_Troop.prototype.rearrangeEnemiesFV = function(dEnemies) {
        ...
        for (let i = 0; i < dEnemies.length; i++) {
            ...
            if (prevX) {
                ...
                if (widths[i] >= 800) {
                    x = prevX + widths[i - 1] + 256; 
                }
                else if(widths[i-1] <= 296) {
                    x = prevX + widths[i - 1] + 128; 
                }
                else {
                    x = prevX + widths[i - 1] + 16; 
                }
                ...
                // 左端
                //敵の数が1体
                if(dEnemies.length == 1) {
                    x = (sideGap - 236) + widths[0] / 2;
                //敵の数が２体
                } else {
                    x = ((sideGap / 3) - 28) + widths[0] / 2;
                }

            }
            prevX = x;

            gEnemy = new Game_Enemy(dEnemies[i].id, x, y + 24);
            if (dEnemies[i].hidden) {
