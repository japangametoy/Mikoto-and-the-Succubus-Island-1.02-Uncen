...
/*:
 * @target MV MZ
 * @plugindesc You will be able to call up the screen to check the map.
 ...
 *  〇 MV / MZ
 *  
 *  〇 CallMenuMap  / callMenuMap
 *   - Call the menu map screen.
 * 
 * ▼ Map notes
 *  〇 <MenuMap:name>
 ...
 * 
 * ▼ Menu map operation method
 *  〇 Move cursor
 *   For pads or keyboards : Arrow keys / Accelerate with shift key
 *   For mouse or touch operation : Move to the touched position
 * 
 *  〇 Scroll map
 *   For pads or keyboards : Arrow keys
 *   For mouse or touch operation : Move while touching
 *  
 *  〇 Map scaling
 *   For pads or keyboards : Page up & down
 *   For mouse : Wheel operation or upper left button
 *   For touch operation : Upper left button
 *   ※ Button display is a function of MZ.
 *  
 *  〇 Point to the pin
 *   For pads or keyboards : Enter key
 *   For mouse or touch operation : After touching, release without moving
 * 
 * ▼ Display name
 *  - The name of the character on which the cursor is placed is displayed.
 *  - Some control characters can be used in the name. (\V,\N,\P,\G,\C,\\)
 *  - For events, if the name is blank '', the default name 'EV???'
 *    (? Is a number), or if it starts with '#', it will not be displayed.
 *
 * ▼ Pin pointing
 *  - If you point to a pin on the menu map screen, the pin will also
 *    be displayed on the minimap.
 *  - Pins are erased as the player approaches.
 * 
 * ================================
 ...
 * (Same as MPP_MiniMap.js / Applies only to menu screen)
 *      @type number
 *          @min 1
 *          @max 999
 *      @default 6
 ...
 *
 *  @param Min Zoom
 *      @desc It will not be less than the full display.
 *      @type number
 *          @min 0
 *          @max 10000
 *      @default 0
 ...
 *      @desc Enable / disable the function that allows you to set a pin with the enter key
 *      @type boolean
 *      @default true
 *
 *  @param Pin Image
 ...
 *      @desc Color when automatically generated
 ...
 *      @desc 
 *      @type struct<Vehicle>
 *      @default {"boat":"Boat","ship":"Ship","airship":"Airship"}
 *      @parent Name Window Position
 * 
...
(() => {
    ...
    Sprite_MinimapMarker.prototype.paintIcon = function(x, y, iconFileName, angle) {
        ...
        if (icon && icon.isReady()) {
            ...
            // マップ全体表示時のサイズ、そうでない場合は元のサイズを使用
            const targetWidth = Game_Minimap.selecting ? 24 : icon.width;
            const targetHeight = Game_Minimap.selecting ? 24 : icon.height;
    
            context.save();
            context.translate(x, y);
            context.rotate(angle * Math.PI / 180);
            context.drawImage(icon._canvas || icon._image, -targetWidth / 2, -targetHeight / 2, targetWidth, targetHeight);
            context.restore();
    ...
    Sprite_MinimapMarker.prototype.painthole = function(x, y, color) {
        ...
        const size = Game_Minimap.selecting ? 10 : 16
        console.log(size)
        const context = this.bitmap.context;
        context.save();
    ...
    Sprite_MinimapMarker.prototype.paintIconImg = function(x, y, imgId, angle) {
        const icon = ImageManager.loadPicture(imgId);
        const icon_switchs = ["スイッチG","スイッチG右","スイッチG下","スイッチG左","スイッチO","スイッチO右","スイッチO下","スイッチO左"]
        if (icon && icon.isReady()) {
            const context = this.bitmap.context;
            const iconWidth = Game_Minimap.selecting ? 24 : icon.width;
            const iconHeight = Game_Minimap.selecting ? 24 : icon.height;
            ...
            context.drawImage(icon._canvas || icon._image, -iconWidth / 2, -iconHeight / 2, iconWidth, iconHeight);
            context.restore();
    ...
    Sprite_MinimapMarker.prototype.drawOneWayDoor = function(x, y, color, angle, wall) {
        ...
        // Game_Minimap.selectingを使用してマップ全体表示かどうかを判定
        let scale = 0.6; // マップ全体表示時は40%の大きさに、そうでなければ60%に
        let scale2 = 0.35;
        if(Game_Minimap.selecting) {
            switch ($dataMap.width) {
                case 75:
                    scale = 0.6;
                    break;
                case 91:
        ...
        if (wall === 0) {
            ...
            // }
            width = 105 * scale2;
            height = 10 * scale; // 横棒の高さを調整
            space = 25 * scale; // 中央のスペースの幅を調整
            ...
            barWidth = 7 * scale; // 横棒の高さ
            barHeight = 20 * scale; // 横棒の幅を調整
          
            // 横棒を描画
            context.fillRect(x - width / 2 - barWidth / 2, y - barHeight / 2, barWidth, barHeight);
            context.fillRect(x + width / 2 - barWidth / 2, y - barHeight / 2, barWidth, barHeight);
        } else {
            width = 10 * scale; // 縦棒の幅を調整
            height = 105 * scale2;
            // 縦棒の高さ
            // if (Game_Minimap.selecting){
            //     height = 105 * 0.25;
            ...
            barWidth = 20 * scale; // 横棒の幅を調整
            barHeight = 7 * scale; // 横棒の高さ
            // 横棒を描画
        ...
        // 矢印の棒部分を描画
        context.beginPath();
        context.moveTo(0, -15 * scale); // 40%または60%に調整
        context.lineTo(0, 15 * scale); // 同上
        context.strokeStyle = 'rgba(209, 246, 255, 1)';
        context.lineWidth = 10 * scale; // 線の太さも調整
        context.stroke();
    
        // 矢印の先端を描画
        context.beginPath();
        context.moveTo(0, -30 * scale); // 先端の長さを調整
        context.lineTo(12 * scale, -12 * scale); // 右側の先端も調整
        context.lineTo(-12 * scale, -12 * scale); // 左側の先端も調整
        context.closePath();
    ...
    Sprite_MinimapMarker.prototype.paintHorizontalBarY = function(x, y, index) {
        let width = 105; // 横向きの扉の幅
        let height = 10; // 横向きの扉の高さ
        const context = this.bitmap.context;
        let width_scale = 0.35;
        let height_scale = 0.60;
        ...
        width *= width_scale; // 例: 元の値より小さくする
        height *= height_scale; // 例: 元の値より小さくする
        ...
        // if (Game_Minimap.selecting) {
        //     // マップ全体表示時のサイズ調整
        //     width *= 0.43; // 例: 元の値より小さくする
        //     height *= 0.60; // 例: 元の値より小さくする
        // } else {
        //     // 通常表示時のサイズ調整
        //     width *= 0.35;
        //     height *= 0.60;
        ...
        let barHeight = 20; // 横棒の幅
    
        // Game_Minimap.selectingを使用してマップ全体表示かどうかを判定
        if (Game_Minimap.selecting) {
            // マップ全体表示時のサイズ調整
            barWidth *= 0.5; // 例: 元の値より小さくする
            barHeight *= 0.5; // 例: 元の値より小さくする
        } else {
            // 通常表示時のサイズ調整
            barWidth *= 0.5;
            barHeight *= 0.5;
    ...
    Sprite_MinimapMarker.prototype.paintHorizontalBarZ = function(x, y, index) {
        let width = 10; // 横棒の幅
        let height = 105; // 横棒の高さ
        const context = this.bitmap.context;

        let width_scale = 0.60;
        let height_scale = 0.35;
        ...
        // if (Game_Minimap.selecting) {
        //     // マップ全体表示時のサイズ調整
        //     width *= 0.60; // 例: 元の値より小さくする
        //     height *= 0.43; // 例: 元の値より小さくする
        // } else {
        //     // 通常表示時のサイズ調整
        //     width *= 0.60;
        //     height *= 0.35;
        ...
        let barWidth = 20; // 横棒の高さ
        let barHeight = 7; // 横棒の幅
    ...
    //         try {
    //             // ミニマップが表示されている場合かつ、戦闘中以外の場合のみマップの全体表示を行う。
    //             if ($gameMap.minimap._paramsIndex !== 0 && !$gameParty.inBattle()) {
    //                 if (SceneManager._scene instanceof Scene_MenuMap) {
    //                     SceneManager.pop();
    //                 } else {
    //                     SceneManager.push(Scene_MenuMap);
    //                 }
    ...
    Window_MenuCommand.prototype.addOriginalCommands = function() {
        ...
        // ミニマップが表示されている場合のみ項目追加
        if (param_MenuCommandText && $gameMap.minimap._paramsIndex !== 0) {
