 ...
 * @param newSwitch
 * @text なんとかスイッチ
 * @desc なんとかスイッチオプションに関する設定です。
 * 
 * @param useNewSwitch
 * @text なんとかスイッチの使用
 * @desc なんとかスイッチを使用するかどうかを設定してください。
 * @parent newSwitch
 * @default true
 * @type boolean
 * 
 * @param defaultNewSwitch
 * @text デフォルトなんとかスイッチ
 * @desc なんとかスイッチのデフォルト値です。
 * @parent newSwitch
 * @default false
 * @type boolean
 * 
...
(() => {
    ...
    //switc 
    const NEW_SWITCH_NAME = "なんとかスイッチ";
    const USE_NEW_SWITCH = pluginParams.useNewSwitch === "true";
    const DEFAULT_NEW_SWITCH = pluginParams.defaultNewSwitch === "true";

    const NEW_SWITCH_ID = Number(pluginParams.newSwitchID);
    ...
    ConfigManager.makeData = function() {
        ...
        config.newSwitch = this.newSwitch;
        return config;
    };
    ...
    ConfigManager.applyData = function(config) {
        ...
        this.newSwitch = this.readFlag(config, "newSwitch", DEFAULT_NEW_SWITCH);
    };
    ...
    Game_Player.prototype.update = function(sceneActive) {
        ...
        if(USE_NEW_SWITCH) {
            if (ConfigManager.newSwitch) {
                $gameSwitches.setValue(NEW_SWITCH_ID, true);
            } else {
                $gameSwitches.setValue(NEW_SWITCH_ID, false);
            }
        }
    };
    ...
    Window_Options.prototype.addBooleanOptions = function() {
        ...
        if (USE_NEW_SWITCH) this.addCommand(NEW_SWITCH_NAME, "newSwitch");
    };
    ...
    Window_Options.prototype.isBooleanSymbol = function(symbol) {
        return ["alwaysDash", "commandRemember", "touchUI", "switchABButtons", "fastMessage","newSwitch"].includes(symbol);
    };
    ...
    Window_Options.prototype.restoreDefaultValues = function() {
        ...
        ConfigManager["newSwitch"] = DEFAULT_NEW_SWITCH;
        ConfigManager["voiceVolume"] = DEFAULT_VOLUME;
        // 追加オプション項目のデフォルト値を設定
        const customParams = ConfigManager.getCustomParams();
        for (const symbol in customParams) {
            if (customParams.hasOwnProperty(symbol)) {
                const item = customParams[symbol];
                ConfigManager[symbol] = item.initValue;
            }
        }
