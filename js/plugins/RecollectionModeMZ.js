...
//-----------------------------------------------------------------------------
    ...
    Scene_Recollection.prototype.createRecollectionCommandRect = function() {
        ...
        const wx = (Graphics.boxWidth - ww) / 2; // 画面の幅からウィンドウの幅を引いて半分にする
        const wy = (Graphics.boxHeight - wh) / 2; // 画面の高さからウィンドウの高さを引いて半分にする
...
//-----------------------------------------------------------------------------
    ...
    Window_RecList.prototype.itemHeight = function() {
        const height = (this.height - 86) / rngdRecollectionModeMZSettings["recModeList"]["recModeListItemRow"];
    ...
    Window_RecList.prototype.maxCols = function() {
        const cols = rngdRecollectionModeMZSettings["recModeList"]["recModeListItemColumn"];
        //console.log("Max Columns:", cols);
        return cols;
    };
    ...
    Window_RecList.prototype.itemWidth = function() {
        return 420; // 幅を420に設定
    };
    ...
    Window_RecList.prototype.itemRect = function(index) {
        ...
        // 各枠の表示位置を設定
        if (col === 0) {
            rect.x = 70; // 最初の枠はX座標を0から+70ピクセル
        }else{
            rect.x = 70 + col * (rect.width + 20); // 3番目の枠は+50ピクセル
        }
        if (row === 0) {
            rect.y = 86; // 最初の枠はX座標を0から+70ピクセル
        }else{
            rect.y = 86 + row * (rect.height + 86); // 3番目の枠は+50ピクセル
        }

        //rect.x = index % maxCols * (rect.width + this.spacing()) + xOffset;  // x座標を調整
    ...
    Window_RecList.prototype.drawItem = function(index) {
        ...
        // Adjusting the dimensions of the frame
        const frameWidth = 420;
        const frameHeight = 245;
        const imageWidth = 400;
        const imageHeight = 225;
        
        // Centering the image within the frame
        const dx = (frameWidth - imageWidth) / 2; // 中心に配置するためのX座標のオフセット
        const dy = (frameHeight - imageHeight) / 2; // 中心に配置するためのY座標のオフセット

        // CG set's switch number is true across all save data, display the picture
        if(rngdRecollectionGlobalContents["switches"][rec_cg.switch_id]) {
            ...
            this.drawRecollection(thumbnail_file_name, 0, 0, imageWidth, imageHeight, rect.x+dx, rect.y+dy);
        } else {
            this.drawRecollection(rngdRecollectionModeMZSettings["recModeList"]["recModeListNeverWatchPictureName"],
                    0, 0 , imageWidth, imageHeight, rect.x+dx, rect.y+dy);
        }
    };
...
//-----------------------------------------------------------------------------
    ...
    Window_TitleCommand.prototype.makeCommandList = function() {
        ...
        this.addCommand('終了', 'exit'); // '終了'コマンドを追加
    };
    ...
    Scene_Title.prototype.createCommandWindow = function() {
        ...
        this._commandWindow.setHandler('exit', this.commandExit.bind(this)); // '終了'コマンドに対するハンドラーを設定
    };

    // '終了'コマンドが選択されたときの処理を追加
...
// // Window_RecListクラスに追加
// Window_RecList.prototype.processCursorMove = function() {
...
// Window_RecList.prototype.cursorDown = function() {
//     // 上下のカーソル移動を無効にする
// };

// Window_RecList.prototype.cursorUp = function() {
//     // 上下のカーソル移動を無効にする
// };
